...
def get_bmi_category():
    ...
    weight_pounds = float(input("Please enter the weight in pounds"))
...
def get_discount():
    ...
    """
    #### write your solution for this function below here. ####
    number_masks = int(input("Please enter the number of masks needed"))
    cost = number_masks * 5
    discounted_cost = "{:,}".format(int(round(cost * 0.8)))
    undiscounted_cost = "{:,}".format(int(round(cost)))
    if number_masks >= 1000:
        cost_for_return = "$" + str(discounted_cost)
        return cost_for_return
    else:
        cost_for_return = "$" + str(undiscounted_cost)
        return cost_for_return
